...
function executeDispatch(event, listener, inst) {
  ...
  event.currentTarget = inst.tag !== undefined ? getNodeFromInstance(inst) : inst;
...
var TOP_AFTER_BLUR = unsafeCastStringToDOMTopLevelType('afterblur');
var TOP_BEFORE_BLUR = unsafeCastStringToDOMTopLevelType('beforeblur'); // List of events that need to be individually attached to media elements.
...
function attemptReplayContinuousQueuedEvent(queuedEvent) {
  ...
  while (targetContainers.length > 0) {
    ...
    var nextBlockedOn = attemptToDispatchEvent(queuedEvent.topLevelType, queuedEvent.eventSystemFlags, targetContainer, queuedEvent.nativeEvent);
...
function replayUnblockedEvents() {
  ...
  while (queuedDiscreteEvents.length > 0) {
    ...
    while (targetContainers.length > 0) {
      var targetContainer = targetContainers[0];
      var nextBlockedOn = attemptToDispatchEvent(nextDiscreteEvent.topLevelType, nextDiscreteEvent.eventSystemFlags, targetContainer, nextDiscreteEvent.nativeEvent);
...
function accumulateTwoPhaseListeners(event, accumulateUseEventListeners, eventSystemFlags, targetContainer) {
  var phasedRegistrationNames = event.dispatchConfig.phasedRegistrationNames;
  var dispatchListeners = [];
  var dispatchInstances = []; // For TargetEvent only accumulation, we do not traverse through
...
var SimpleEventPlugin = {
  ...
  extractEvents: function (topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
    ...
    switch (topLevelType) {
      ...
      default:
        {
          if (knownHTMLTopLevelTypes.indexOf(topLevelType) === -1 && dispatchConfig.customEvent !== true) {
...
function updateSuspenseState(prevSuspenseState, renderExpirationTime) {
  var prevSuspendedTime = prevSuspenseState.baseTime;
  return {
    dehydrated: null,
    baseTime: // Choose whichever time is inclusive of the other one. This represents
    // the union of all the levels that suspended.
    prevSuspendedTime !== NoWork && prevSuspendedTime < renderExpirationTime ? prevSuspendedTime : renderExpirationTime,
    retryTime: NoWork
  };
}

function shouldRemainOnFallback(suspenseContext, current, workInProgress, renderExpirationTime) {
  ...
  if (current !== null) {
    ...
    if (suspenseState !== null) {
      ...
      var baseTime = suspenseState.baseTime;
...
function getRemainingWorkInPrimaryTree(current, workInProgress, currentPrimaryChildFragment, renderExpirationTime) {
  var currentParentOfPrimaryChildren = currentPrimaryChildFragment !== null ? currentPrimaryChildFragment : current;
  var currentChildExpirationTime = currentParentOfPrimaryChildren.childExpirationTime;
  var currentSuspenseState = current.memoizedState;
  ...
  if (currentSuspenseState !== null) {
    ...
    var baseTime = currentSuspenseState.baseTime;

    if (baseTime !== NoWork && baseTime < renderExpirationTime && baseTime > currentChildExpirationTime) {
...
function updateSuspenseComponent(current, workInProgress, renderExpirationTime) {
  ...
  if (didSuspend || shouldRemainOnFallback(suspenseContext, current, workInProgress, renderExpirationTime)) {
    ...
    if (prevState !== null) {
      {
        ...
        if (_dehydrated !== null) {
          if (!didSuspend) {
            ...
            _primaryChildFragment.childExpirationTime = getRemainingWorkInPrimaryTree(current, workInProgress, null, renderExpirationTime);
            workInProgress.memoizedState = updateSuspenseState(current.memoizedState, renderExpirationTime);
      ...
      if (nextDidTimeout) {
        ...
        _primaryChildFragment2.childExpirationTime = getRemainingWorkInPrimaryTree(current, workInProgress, currentPrimaryChildFragment, renderExpirationTime); // Skip the primary children, and continue working on the
        // fallback children.
      ...
      if (nextDidTimeout) {
        ...
        _primaryChildFragment3.childExpirationTime = getRemainingWorkInPrimaryTree(current, workInProgress, null, renderExpirationTime); // Skip the primary children, and continue working on the
        // fallback children.
            ...
            if (state !== null) {
              ...
              if (primaryChildExpirationTime !== NoWork && primaryChildExpirationTime >= renderExpirationTime) {
                ...
                while (primaryChild !== null) {
                  var childUpdateExpirationTime = primaryChild.expirationTime;
                  var childChildExpirationTime = primaryChild.childExpirationTime;

                  if (childUpdateExpirationTime !== NoWork && childUpdateExpirationTime >= renderExpirationTime || childChildExpirationTime !== NoWork && childChildExpirationTime >= renderExpirationTime) {
...
function attachPingListener(root, renderExpirationTime, wakeable) {
  ...
  if (!threadIDs.has(renderExpirationTime)) {
    ...
    var ping = pingSuspendedRoot.bind(null, root, wakeable, renderExpirationTime);
...
function warnAboutUpdateOnUnmountedFiberInDEV(fiber) {
  {
    ...
    if (isFlushingPassiveEffects) {
      ...
      var currentTargetFiber = fiber;

      while (currentTargetFiber !== null) {
        // currentDebugFiberInDEV owns the effect destroy function currently being invoked.
        if (current === currentTargetFiber || current === currentTargetFiber.alternate) {
          error("Can't perform a React state update from within a useEffect cleanup function. " + 'To fix, move state updates to the useEffect() body.%s', getStackByFiberInDevAndProd(current));
